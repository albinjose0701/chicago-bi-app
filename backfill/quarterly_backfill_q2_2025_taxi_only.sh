#!/bin/bash
#
# Quarterly Backfill Script - Q2 2025 (Apr 1 - Jun 30)
...
#
# This script extracts taxi trip data for Q2 2025 (91 days)
...
# Logging
LOG_FILE="backfill_q2_2025_taxi_only_$(date +%Y%m%d_%H%M%S).log"
PROGRESS_FILE="q2_2025_taxi_progress.txt"
...
echo -e "${BLUE}════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Q2 2025 TAXI BACKFILL (Apr 1 - Jun 30)${NC}"
echo -e "${BLUE}  NEW DATASET: ajtu-isnz${NC}"
echo -e "${BLUE}════════════════════════════════════════════════${NC}"
echo ""
echo -e "Configuration:"
echo -e "  • Period: ${GREEN}2025-04-01 to 2025-06-30${NC}"
...
echo ""
echo -e "${BLUE}════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Q2 2025 BACKFILL COMPLETE${NC}"
echo -e "${BLUE}════════════════════════════════════════════════${NC}"
echo ""
echo -e "Summary:"
...
if [[ $FAILED -eq 0 ]]; then
    print_success "🎉 Q2 2025 backfill completed successfully!"
    exit 0
else
    print_error "⚠️  Q2 2025 backfill completed with ${FAILED} failures"
    exit 1
fi
