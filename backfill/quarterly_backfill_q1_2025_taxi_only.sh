#!/bin/bash
#
# Quarterly Backfill Script - Q1 2025 (Jan 1 - Mar 31)
...
#
# This script extracts taxi trip data for Q1 2025 (90 days)
...
# Logging
LOG_FILE="backfill_q1_2025_taxi_only_$(date +%Y%m%d_%H%M%S).log"
PROGRESS_FILE="q1_2025_taxi_progress.txt"
...
echo -e "${BLUE}════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Q1 2025 TAXI BACKFILL (Jan 1 - Mar 31)${NC}"
echo -e "${BLUE}  NEW DATASET: ajtu-isnz${NC}"
echo -e "${BLUE}════════════════════════════════════════════════${NC}"
echo ""
echo -e "Configuration:"
echo -e "  • Period: ${GREEN}2025-01-01 to 2025-03-31${NC}"
echo -e "  • Days: ${GREEN}90 days${NC}"
...
while [[ "$CURRENT_DATE" < "$END_DATE" ]] || [[ "$CURRENT_DATE" == "$END_DATE" ]]; do
    ...
    echo -e "${BLUE}──────────────────────────────────────────────${NC}"
    echo -e "${BLUE}Date: ${CURRENT_DATE} (${TOTAL_DATES}/90)${NC}"
    echo -e "${BLUE}──────────────────────────────────────────────${NC}"
...
echo ""
echo -e "${BLUE}════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Q1 2025 BACKFILL COMPLETE${NC}"
echo -e "${BLUE}════════════════════════════════════════════════${NC}"
echo ""
echo -e "Summary:"
...
if [[ $FAILED -eq 0 ]]; then
    print_success "🎉 Q1 2025 backfill completed successfully!"
    exit 0
else
    print_error "⚠️  Q1 2025 backfill completed with ${FAILED} failures"
    exit 1
fi
