#!/bin/bash
#
# Chicago BI App - Quarterly Backfill Script v2.2.0
...
#
# Usage:
#   ./quarterly_backfill_q2_2020.sh [dataset]
#
# Arguments:
#   dataset: taxi, tnp, or "all" (default: all)
#
# Examples:
#   ./quarterly_backfill_q2_2020.sh taxi       # Only taxi trips
#   ./quarterly_backfill_q2_2020.sh all        # All datasets
#
...
echo -e "${BLUE}================================================${NC}"
echo -e "${BLUE}Chicago BI App - Q2 2020 Quarterly Backfill v2.2.0${NC}"
echo -e "${BLUE}================================================${NC}"
echo ""
echo -e "Quarter: ${GREEN}Q2 2020 (Apr 1 - Jun 30)${NC}"
echo -e "Partitions: ${GREEN}91 daily partitions${NC}"
echo -e "Dataset: ${GREEN}${DATASET}${NC}"
echo -e "Project: ${GREEN}${PROJECT_ID}${NC}"
echo -e "Note: ${YELLOW}Q2 includes COVID-19 pandemic impact period${NC}"
echo ""
...
# Generate array of all dates in Q2 2020
generate_date_range() {
    local start_date=$1
    local end_date=$2
...
run_quarterly_backfill() {
    ...
    print_section "Generating Date Range for Q2 2020"
    ...
    # Create log file
    log_file="backfill_q2_2020_${dataset_type}_$(date +%Y%m%d_%H%M%S).log"
    echo "Quarterly Backfill Log - Q2 2020 v2.2.0" > "$log_file"
...
# Confirmation prompt
echo ""
echo -e "${YELLOW}⚠️  WARNING: This will execute Cloud Run jobs for 91 days of data.${NC}"
echo -e "${YELLOW}   Q2 2020 includes significant COVID-19 pandemic impact.${NC}"
echo -e "${YELLOW}   Expect reduced trip volumes compared to Q1.${NC}"
echo -e "${YELLOW}   Each extraction will be verified for data completeness.${NC}"
echo -e "${YELLOW}   Estimated cost: ~\$3-4 (one-time)${NC}"
echo -e "${YELLOW}   Estimated time: ~$(( (DELAY_SECONDS + 30) * 91 / 60 )) minutes per dataset${NC}"
echo ""
...
echo "   bq query --use_legacy_sql=false \"SELECT DATE(trip_start_timestamp) AS date, COUNT(*) as trips FROM \\\`${PROJECT_ID}.raw_data.raw_taxi_trips\\\` WHERE DATE(trip_start_timestamp) BETWEEN '2020-04-01' AND '2020-06-30' GROUP BY date ORDER BY date\""
echo ""
echo "2. Check partition count:"
echo "   bq query --use_legacy_sql=false \"SELECT COUNT(DISTINCT DATE(trip_start_timestamp)) as partition_count FROM \\\`${PROJECT_ID}.raw_data.raw_taxi_trips\\\` WHERE DATE(trip_start_timestamp) BETWEEN '2020-04-01' AND '2020-06-30'\""
echo ""
echo "3. Compare Q1 vs Q2 volumes (COVID impact analysis):"
echo "   bq query --use_legacy_sql=false \"SELECT 'Q1' as quarter, COUNT(*) as trips FROM \\\`${PROJECT_ID}.raw_data.raw_tnp_trips\\\` WHERE DATE(trip_start_timestamp) BETWEEN '2020-01-01' AND '2020-03-31' UNION ALL SELECT 'Q2', COUNT(*) FROM \\\`${PROJECT_ID}.raw_data.raw_tnp_trips\\\` WHERE DATE(trip_start_timestamp) BETWEEN '2020-04-01' AND '2020-06-30'\""
echo ""
