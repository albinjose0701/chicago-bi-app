#!/bin/bash
#
# Quarterly Backfill Script - Q3 2025 (Jul 1 - Oct 1)
...
#
# This script extracts taxi trip data for Q3 2025 (93 days)
...
# Logging
LOG_FILE="backfill_q3_2025_taxi_only_$(date +%Y%m%d_%H%M%S).log"
PROGRESS_FILE="q3_2025_taxi_progress.txt"
...
echo -e "${BLUE}════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Q3 2025 TAXI BACKFILL (Jul 1 - Oct 1)${NC}"
echo -e "${BLUE}  NEW DATASET: ajtu-isnz${NC}"
echo -e "${BLUE}════════════════════════════════════════════════${NC}"
echo ""
echo -e "Configuration:"
echo -e "  • Period: ${GREEN}2025-07-01 to 2025-10-01${NC}"
echo -e "  • Days: ${GREEN}93 days${NC}"
...
while [[ "$CURRENT_DATE" < "$END_DATE" ]] || [[ "$CURRENT_DATE" == "$END_DATE" ]]; do
    ...
    echo -e "${BLUE}──────────────────────────────────────────────${NC}"
    echo -e "${BLUE}Date: ${CURRENT_DATE} (${TOTAL_DATES}/93)${NC}"
    echo -e "${BLUE}──────────────────────────────────────────────${NC}"
...
echo ""
echo -e "${BLUE}════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Q3 2025 BACKFILL COMPLETE${NC}"
echo -e "${BLUE}════════════════════════════════════════════════${NC}"
echo ""
echo -e "Summary:"
...
if [[ $FAILED -eq 0 ]]; then
    print_success "🎉 Q3 2025 backfill completed successfully!"
    exit 0
else
    print_error "⚠️  Q3 2025 backfill completed with ${FAILED} failures"
    exit 1
fi
