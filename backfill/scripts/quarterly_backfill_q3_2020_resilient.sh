#!/bin/bash
#
# Resilient Q3 2020 Quarterly Backfill with Network Failure Recovery
# OPTIMIZED: 10s delays (3x faster than Q2)
...
# Delays - OPTIMIZED for faster execution
DELAY_SECONDS=10  # Reduced from 30s (3x faster)
NETWORK_RETRY_DELAY=120
MAX_NETWORK_RETRIES=10
MAX_EXTRACTION_RETRIES=3

# Create log file with timestamp
LOG_FILE="backfill_q3_2020_resilient_$(date +%Y%m%d_%H%M%S).log"
PROGRESS_FILE="q3_progress.txt"
...
echo -e "${BLUE}================================================${NC}"
echo -e "${BLUE}Resilient Q3 2020 Quarterly Backfill (OPTIMIZED)${NC}"
echo -e "${BLUE}With Network Failure Recovery${NC}"
echo -e "${BLUE}================================================${NC}"
echo ""
echo -e "Quarter: ${GREEN}Q3 2020 (Jul 1 - Sep 30)${NC}"
echo -e "Date Range: ${GREEN}${START_DATE} to ${END_DATE}${NC}"
echo -e "Datasets: ${GREEN}${DATASET}${NC}"
echo -e "Project: ${GREEN}${PROJECT_ID}${NC}"
echo -e "Delay: ${YELLOW}10 seconds (OPTIMIZED - 3x faster)${NC}"
...
# Main backfill logic
run_backfill() {
    print_info "Generating date range for Q3 2020..."
    ...
    if [[ $failed -eq 0 ]]; then
        print_success "Q3 2020 backfill completed successfully!"
        return 0
    else
        print_error "Q3 2020 backfill completed with ${failed} failures"
        return 1
    fi
}
